...
def newtrip(message):
    ...
    button1 = InlineKeyboardButton("Add me to the trip", callback_data="add_to_trip")
...
@bot.callback_query_handler(func=lambda call: call.data == "add_to_trip")
def add_to_trip(call):
    ...
    if users.find_one({"username": username, "chatid": chat_id}) == None:
        users.insert_one({"chatid": chat_id, "amount_spent": 0, "amount_paid": 0 ,"username": username})
...
def overview(message):
    ...
    chat_id = message.chat.id
    results = users.find({"chatid": chat_id})
    response = ""
    counter = 1
    ...
    #     # If the person has a positive balance, they owe money
    #     if balance > 0:
    ...
    #                 if balance2 < 0:
    #                     # If the other person has a negative balance, they need to be paid
    ...
    for result in results :
        response += str(counter) + ". " + result["username"] + "\n"
        response += "Amount Spent: <b>" + str(result["amount_spent"]) + "</b>\n"
        response += "Amount Paid: <b>" + str(result["amount_paid"]) + "</b>\n\n"
        counter = counter + 1
